...
class UltraLight_VM_UNet(nn.Module):
    ...
    #             split_att='fc', bridge=True, half=False):
    def __init__(self, input_channels=3, c_list=[16,48,96,192,256,384],
            split_att='fc', bridge=True, half=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        t6 = F.gelu(self.encoder6(out)) # b, c5, H/32, W/32
...
if __name__=="__main__":
    init=torch.randn(16,3,640,640).cuda()
    models=UltraLight_VM_UNet().cuda()
    out=models(init)
    for i in out:
        print(i.shape)
